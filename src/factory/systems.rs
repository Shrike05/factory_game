...
pub fn create_factory_assets(
    ...
    meshes.insert(FactoryName::from_string("Empty"), mesh);
    let mut materials = HashMap::new();
    materials.insert(FactoryName::from_string("Empty"), w_material);
...
pub fn spawn_factories(
    ...
    for message in msg.read() {
        ...
        let shape = &get_factory_attributes(&message.factory_type).shape;
        for offset in shape {
            build_map
